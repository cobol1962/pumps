...
$sit = implode(",", $sites);
$sql = "select distinct siteid from fuel_sales where siteid IN($sit) and submitted=1 order by fuelid";
$r = $mysqli->query($sql);
if ($timeline == "sum") {
  if ($grouping == "sites") {
      while ($row = mysqli_fetch_assoc($r)) {
        ...
        $site = $row["siteid"];
        foreach ($methods as $m) {
          $sum = "select IFNULL(sum(value),0) as s from fuel_sales where submitted=1 and siteid=$site and fuelid=$m AND (date >= '$datefrom' AND date <= '$dateto')";
    ...
    foreach ($methods as $m) {
      $sum = "select IFNULL(sum(value),0) as s from fuel_sales where submitted=1 and siteid IN ($sit) AND fuelid=$m AND (date >= '$datefrom' AND date <= '$dateto')";
      $rsum = $mysqli->query($sum);
      $table[$site][$m] = floatval(mysqli_fetch_assoc($rsum)["s"]);
    }
  }
...
if ($timeline != "sum") {
  if ($grouping == "sites") {
      while ($row = mysqli_fetch_assoc($r)) {
        ...
        foreach ($methods as $m) {
          if ($timeline == "day") {
            $sum = "select date,siteid,IFNULL(sum(value),0) as s from fuel_sales where submitted=1 and siteid=$site and fuelid=$m AND (date >= '$datefrom' AND date <= '$dateto') group by date order by date";
          }
          if ($timeline == "week") {
            $sum = "select CONCAT('Week ',week(date)) as date,siteid,IFNULL(sum(value),0) as s from fuel_sales where submitted=1 and siteid=$site and fuelid=$m AND (date >= '$datefrom' AND date <= '$dateto') group by week(date) order by date";
          }
          if ($timeline == "month") {
            $sum = "select CONCAT('Month ',month(date)) as date,siteid,IFNULL(sum(value),0) as s from fuel_sales where submitted=1 and  siteid=$site and fuelid=$m AND (date >= '$datefrom' AND date <= '$dateto') group by month(date) order by date";
          }
          $rsum = $mysqli->query($sum);
...
////////////////////////////////////////////izmene OVDE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    foreach ($methods as $m) {
      if ($timeline == "day") {
        $sum = "select date,IFNULL(sum(value),0) as s from fuel_sales where submitted=1 and siteid in($sit) AND fuelid=$m AND (date >= '$datefrom' AND date <= '$dateto') Group By date";
      }

      if ($timeline == "week") {
        $sum = "select CONCAT('Week ',week(date)) as date,IFNULL(sum(value),0) as s from fuel_sales where submitted=1 and  siteid in($sit) AND fuelid=$m AND (date >= '$datefrom' AND date <= '$dateto') Group By Week(date)";
      }
      if ($timeline == "month") {
        $sum = "select CONCAT('Month ',month(date)) as date,IFNULL(sum(value),0) as s from fuel_sales where submitted=1 and siteid in($sit) AND  fuelid=$m AND (date >= '$datefrom' AND date <= '$dateto') Group By month(date)";
      }
      $rsum = $mysqli->query($sum);
      while ($row = mysqli_fetch_assoc($rsum)) {
