  ...
  if ($grouping == "sites") {
      while ($row = mysqli_fetch_assoc($r)) {
        ...
        foreach ($methods as $m) {
          ...
          if ($timeline == "week") {
            $sum = "select CONCAT('Week ',week(date)) as date,siteid,IFNULL(sum(value),0) as s,sqlite_fetch_column_types
             from `sales_oil_lottery` from `sales_oil_lottery`  left join oil_lottery on `sales_oil_lottery`.oilid=oil_lottery.oilid
             where submitted=1 and siteid=$site and type=$m AND (date >= '$datefrom' AND date <= '$dateto') group by week(date) order by date";
          }
          if ($timeline == "month") {
            $sum = "select CONCAT('Month ',month(date)) as date,siteid,IFNULL(sum(value),0) as s,sqlite_fetch_column_types
            from `sales_oil_lottery`  left join oil_lottery on `sales_oil_lottery`.oilid=oil_lottery.oilid
...
////////////////////////////////////////////izmene OVDE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    foreach ($methods as $m) {
      ...
      if ($timeline == "week") {
        $sum = "select CONCAT('Week ',week(date)) as date,IFNULL(sum(value),0) as s from `sales_oil_lottery` where submitted=1 and  siteid in($sit) AND type=$m AND (date >= '$datefrom' AND date <= '$dateto') Group By Week(date)";
      }
      if ($timeline == "month") {
